...
export default function LeadProfilePage() {
  ...
  async function saveDisposition(disp, callStatus, date, slot, scheduleType) {
    ...
    if (scheduleType === 'meeting' && date && slot) {
      ...
      // Agar qc_followup (Rearrange Meeting) se wapas meeting schedule ho rahi hai
      // toh sales outcome clear karo — sales agent ko fresh start milega
      if (lead.stage === 'qc_followup') {
        updates.sales_outcome = null
        updates.sales_lead_status = null
        updates.sales_meeting_status = null
        updates.sales_callback_date = null
        updates.sales_callback_slot = null
        updates.sales_followup_date = null
    ...
    // PS agent + meeting_scheduled stage + disposition already set (meeting hua tha)
    // → dobara change karna hai → manager approval chahiye
    // Manager approve kare toh stage bhi change ho (NQ/NI → sales queue se hat jaaye)
    if (
      isPresales &&
      role === 'presales_agent' &&
      ...
      // Target stage determine karo based on new disposition
      ...
      const { error } = await supabase.from('disposition_approvals').insert({
        ...
        new_call_status: callStatus,
        notes: targetStage ? `stage:${targetStage}` : null,
        status: 'pending',
        requested_at: new Date().toISOString(),
      })
      if (error) {
        alert('Request bhejne mein error: ' + error.message)
      } else {
        alert('✅ Request PS Manager ko bheji gayi — approve hone pe disposition + stage update hoga.')
      }
      setShowDisp(false)
      setSaving(false)
      return
  ...
  return (
    <Layout>
      ...
      <div className="card mb-4 flex flex-wrap items-center gap-2">
        ...
        {(isSales || isSuperAdmin) && lead.stage === 'sale_rejected' && (
          <button onClick={() => setShowOrderDetails(true)}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-medium bg-orange-50 border-orange-300 text-orange-700 hover:bg-orange-100 transition-colors">
            <ClipboardList size={13} /> Re-submit Order Details
          </button>
        )}
      ...
      {showSalesOutcome && (
        <SalesOutcomeModal
          ...
          onSaved={async (updates) => {
            ...
            // If order closed → open order details form for manager approval
            if (updates.stage === 'sale_pending_approval') {
        ...
        <OrderDetailsModal
          ...
          onSaved={(orderData) => {
            setLead(prev => ({
              ...
              // If was rejected, reflect the stage change back to pending
              ...(prev.stage === 'sale_rejected' ? { stage: 'sale_pending_approval', order_rejected_reason: null } : {}),
            }))
...
function SalesOutcomeModal({ lead, onClose, onSaved, userId, userName }) {
  ...
  async function handleSave() {
    ...
    // Multiple NC — lead wapas presales ke paas, meeting clear, sales outcome bhi clear hoga next time
    if (outcome === 'multiple_nc_rearrange') {
      ...
      // Presales disposition bhi update karo — PS agent ko pata chale
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isMeetingDisposition(form.disposition) && form.meeting_date && form.meeting_slot) {
        ...
        // qc_followup (Rearrange Meeting) se wapas meeting schedule → sales data clear
        if (lead.stage === 'qc_followup') {
          payload.sales_outcome = null
          payload.sales_lead_status = null
          payload.sales_meeting_status = null
          payload.sales_callback_date = null
          payload.sales_callback_slot = null
          payload.sales_followup_date = null
          payload.sales_followup_slot = null
        }
      }

      // PS agent + meeting_scheduled pe disposition change → approval required
      const dispositionChanged = form.disposition !== lead.disposition
      const needsApproval =
        role === 'presales_agent' &&
        lead.stage === 'meeting_scheduled' &&
        dispositionChanged &&
        !payload.stage // naya meeting schedule nahi ho raha
      ...
      if (needsApproval) {
        ...
        await supabase.from('disposition_approvals').insert({
          ...
          old_disposition: lead.disposition,
          new_disposition: form.disposition,
          old_call_status: lead.call_status,
          new_call_status: form.call_status,
          notes: targetStage ? `stage:${targetStage}` : null,
          status: 'pending',
          requested_at: new Date().toISOString(),
        })

        // Baaki fields (non-disposition) save karo normally
...
// ── Order Details Modal ───────────────────────────────────────
function OrderDetailsModal({ lead, onClose, onSaved, userId, userName }) {
  const [saving, setSaving] = useState(false)
